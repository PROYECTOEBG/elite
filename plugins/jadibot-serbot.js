...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  // Manejo especial para el comando code
  if (command.toLowerCase() === 'code') {
    gataJBOptions.args = ['code']
    gataJBOptions.command = 'jadibot'
  }
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    async function connectionUpdate(update) {
      ...
      if (qr && mcode) {
        ...
        if (!secret) {
          return m.reply('No se pudo generar el código de vinculación. Intenta de nuevo.')
        }
        ...
        console.log(`Código generado para ${m.sender}: ${secret}`)
...
setInterval(checkSubBots, 300000); //5 minutos
