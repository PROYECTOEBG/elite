...
// Función optimizada para verificar sub-bots
async function checkSubBots() {
    ...
    for (const folder of subBotFolders) {
        ...
        if (!subBot || !subBot.user) {
            console.log(chalk.blue(`[+] Reconectando sub-bot (+${folder})...`));
            try {
                await gataJadiBot({
                    pathGataJadiBot,
                    m: null,
                ...
                console.error(chalk.red(`[!] Error al reconectar (+${folder}):`), error);
            }
        }
    }
}

// Intervalo de verificación cada 60 segundos (sin cambios)
setInterval(checkSubBots, 60000);

// Manejo de cierre limpio
process.on('SIGINT', () => {
    console.log(chalk.yellow('\n[!] Deteniendo verificador de sub-bots...'));
    process.exit(0);
});
