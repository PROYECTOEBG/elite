...
let handler = async (m, { conn, usedPrefix, command, isAdmin, isBotAdmin, args }) => { if (!isBotAdmin) return conn.reply(m.chat, '⭐ El bot necesita ser administrador.', m); if (!isAdmin) return conn.reply(m.chat, '⭐ Solo los administradores pueden usar este comando.', m);
